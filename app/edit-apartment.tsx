...
import { useApartment } from '@/hooks/useSupabaseQueries';
...
export default function EditApartmentScreen() {
  ...
  const { updateApartment } = useApp();
  const { isSmallScreen, isLargeScreen } = useResponsive();
  const { data: apartment } = useApartment(apartmentId || undefined);
  ...
  useEffect(() => {
    if (apartment && !prefilled) {
      setSourceUrl(apartment.sourceUrl || '');
      setAddress(apartment.address || '');
      setPrice(apartment.price?.toString() || '');
      setBedrooms(apartment.bedrooms?.toString() || '');
      setBathrooms(apartment.bathrooms?.toString() || '');
      setSquareFootage(apartment.squareFootage?.toString() || '');
      setPhotos(apartment.photos || []);
      setListingSource(apartment.listingSource || '');
  ...
  const handleSubmit = async () => {
    if (!apartmentId) {
      showError('No apartment selected');
      return;
    }
    ...
    try {
      ...
      console.error('Update apartment error:', JSON.stringify(error, null, 2));
      if (Platform.OS === 'web') {
        window.alert(`Failed to update apartment: ${message}`);
      } else {
        Alert.alert('Error', `Failed to update apartment: ${message}`);
      }
    } finally {
